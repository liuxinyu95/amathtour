...
The history of number is over 5000 years. In contrast, the history of zero is only about 1200 years. Zero is a new comer. It was originated from India and was introduced to the west through the Arabians. Different from 1, 2, 3, ... zero was created as a placeholder but not a number. It looked like a second level citizen in the kingdom of numbers because zero has no value.
...
There are plenty of things behaving like 0 and 1. For example, we call a sequence of characters a string. Word, phrases, and sentences are strings, hence ``apple'', ``hello'', ``to be or not to be'', `` '' (a space) are all strings. One may concatenate strings, like ``hello'' $\doubleplus$ `` '' = ``hello '', ``hello '' $\doubleplus$ ``apple'' = ``hello apple''. A string containing no character is empty, namely the empty string ``''. Concatenating any string with the empty string leaves that string unchanged:
