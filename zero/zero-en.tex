...
Here are some common notations: Denote natural numbers of 0, 1, 2, 3, ... by $\mathbb{N}$. Before 0 was accepted, natural numbers started from 1, 2, 3, ... people finally recognize 0 is the natural starting point, or origin (see \cref{sec:peano-axioms}); most authors define natural numbers starting from 0 nowadays. We often use $n$ to denote some natural number. The positive numbers, together with zero and negative numbers (e.g., $\dotsc, -3, -2, -1, 0, 1, 2, 3, \dotsc$) are integers, denoted by $\mathbb{Z}$. It comes from the initial letter of the German word `Zahlen' (meaning ``numbers''). We denote positive numbers by $\mathbb{Z}^+$ and negative numbers by $\mathbb{Z}^-$. We sometimes also use $i$ to denote some integer for it is the initial letter of the word `integer'. $i$ is more often used as the index to enumerate things, such as: $a_1, a_2, ..., a_i, ..., a_n$, where $a_i$ is the $i$-th element; $a_n$ is the $n$-th, which is commonly the last.
