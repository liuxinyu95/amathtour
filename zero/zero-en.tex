...
The win of Hindu-Arabic system gave 0 \enquote{citizenship,} legitimate to use. But it's a second class citizen. Why 0 can't be first class as $1, 2, 3, \dotsc$?
...
\item Product. People recognized the notion product from arranging pebbles (or soldiers) in rectangle. $1 \times 5 = 5$ is a rectangle of 1 row and 5 columns; $2 \times 5 = 10$ is a rectangle of 2 rows and 5 columns; $3 \times 5 = 15$ is a rectangle of 3 rows and 5 columns... but what is $0 \times 5$ for? the rectangle vanishes as shown in \cref{fig:grid-vanish}.
...
The product represents rectangle; the ruler represents line segment. Multiplication by 0 destroys them. There was no zero in Greek geometry or numeral system. Euclid defined \enquote{a point is that which has no parts; a line is length without breadth.} (Definition 1 and 2 in Book I of {\em Elements}.) Is 0 a point?
...
When executes the code \lstinline|int a[10]|, the computer will allocate 20 bytes in the memory\footnote{The are two types of memory: stack and heap; It is stack in this example.}, and point \lstinline|a| to the beginning position, called the address. The program next uses this address to index any element of the array. The next two bytes from \lstinline|a| stores the first element; the 3rd and 4th bytes from \lstinline|a| stores the second element. \Cref{fig:array} shows the array storing 1, 2, ..., 10.
...
\begin{example}
In the dating system, 1 CE was the year after Jesus Christ's birth, then followed 2 CE, 3 CE, ... For the historic events happened before Jesus Christ's birth, we mark the year by BCE, for example, 1 BCE, 2 BCE, ... The years of BCE can be viewed as negative years (despite lack of year 0).
\end{example}
...
We may summarize them as: the absolute value of a positive number or zero is itself; the absolute value of a negative number is its inverse. In the influential text-book \emph{Modern Algebra} by Bartel Leendert van der Waerden\footnote{The textbook on abstract algebra is originally based on lectures given by Emil Artin in 1926 and by Emmy Noether from 1924 to 1928. The English translation of 1949 had the title \emph{Modern algebra}, though a later, extensively revised edition in 1970 had the title \emph{Algebra}.} there is a way to \enquote{constructed} integers, including negative numbers and zero, with an ordered pair of natural numbers $(a, b)$\citepage[p8]{van-der-Waerden-2009}.
