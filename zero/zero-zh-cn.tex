...
注意到$2x + 1 = 2(x + \frac{1}{2})$，所以先向\underdot{左}平移$\frac{1}{2}$再放大2倍。一般地，由于$ax + b = a(x + \frac{b}{a})$，故先平移$-\frac{b}{a}$（若$\frac{b}{a}>0$则向左，否则反向）再放大$a$倍。
}
\Question{证明单位元是唯一的。
\begin{proof}
假设存在另一单位元$e'$，使得对任意元素$a$满足$e' \odot a = a \odot e' = a$。我们有：
\begin{align*}
