...
\begin{proof}
Assume there is another representation. If the length are different, we pad with 0 in front of the shorter one to make them same long. Denote the two numbers after padding with zeros as $a_{m} a_{m-1} \cdots a_1 a_0$ and $c_m c_{m-1} \cdots c_1 c_0$. As they have the same level by \cref{eq:pos-rep}:
