...
\Question{利用分数和引理\ref{th:cycle-of-9}解决芝诺悖论中的二分悖论。提示：考虑二进制分数和。\label{qn:dichotomy-paradox}}
...
因此枚举的上限是12。\cref{fig:inherit-program}给出了例子程序。
...
程序对每个可能的$a, b, c$，计算出$n = \frac{abc}{abc - (ab + bc + ac)}$。如果$n$是正整数且能被$a, b, c$都整除。则找到了一个解。下面是一个对应的Haskell例子程序。

\begin{Haskell}[frame = single]
[(a, b, c, n) | a <- [2..12], b <- [a+1..12], c <- [b+1..12],
...
\Question{无限循环小数$0.\dot{a_1} \dot{a_2} \cdots \dot{a_n}$的分数形式是$\frac{a_1 a_2 \cdots a_n}{99 \cdots 9} = \frac{a_1 a_2 \cdots a_n}{10^{n+1} - 1}$。\label{qn:cyclic-decimal}}

\Question{利用分数和解决芝诺悖论中的二分悖论。}
