...
具体怎样约分呢？《九章算术》给出了“辗转相除”法，即著名的欧几里得算法求最大公约数（也叫做最大公因数）：“术曰：可半者半之；不可半者，副置分母、子之数，以少减多，\underdot{更相减损}，求其等也。以等数约之。”解释成现代汉语：首先是特殊情况。若分子分母都是偶数，先不断$\div 2$，化为奇数。然后是一般情况。记分数为$\frac{b}{a}$，记$a$与$b$的最大公约数为$(a, b)$，则：
