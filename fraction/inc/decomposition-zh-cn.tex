...
\begin{enumerate}[1)]
  ...
  \item 如此依次尝试$q, q' = q + 1, q'' = q + 2 \cdots $直到$q + i \leq a$。因为$Q$中的埃及分数是递减的（分母递增），$\frac{b}{a} = \frac{1}{a} + x$中的$x$已经不可能用$n' = b - 1$个小于$\frac{1}{a}$的埃及分数分解。
  \item 每次从$C$的头部取出一对$\frac{b'}{a'}$和$Q$时，我们要比较一下$|Q|$和$|B|$。如果$B \ne \varnothing$并且$Q$中埃及分数更多，它一定不优于分解$B$。并且由于我们每次把包含更多埃及分数的分解放在$C$的末尾，所以$C$中后面的记录一定都比$B$差。此时$B$中记录的就是最优分解。算法结束。
\end{enumerate}
...
\Function{Decompose}{$\frac{b}{a}$}
  ...
  \While{$C \ne \varnothing$}
    \State $(\frac{b}{a}, Q) \gets $ \Call{Pop}{$C$}  \Comment{从头部取出}
    ...
    \EndIf
    \While{$q \leq a$}
      \If{$B \ne \varnothing$ 且 $|Q| \geq |B| - 1$} \Comment{$Q$中已不可能有比$B$更好的分解}
        \State \Return $B$
      \EndIf
      \If{$n'\frac{1}{q} < \frac{b}{a}$} \Comment{$\frac{b}{a}$已无法分解为$n'$个不大于$\frac{1}{q}$的埃及分数}
        \State break
      \EndIf
      ...
      \Else
        \State \Call{Push}{$(\frac{b'}{a'}, \{\frac{1}{q}\} \cup Q), C$} \Comment{记录到末尾}
      \EndIf
      \State $q \gets q + 1$
    \EndWhile
  \EndWhile
