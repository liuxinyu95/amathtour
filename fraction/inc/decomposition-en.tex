...
\begin{enumerate}[1)]
  ...
  \item We next increase $q$ by 1 as $q' = q + 1$, then attempt to decompose $\dfrac{b}{a} = \dfrac{1}{q'} + x$. Obviously, such decomposition isn't as greedy as Fibonacci's with $q$; the remaining part $x$ is greater. Here we need examine if $n\dfrac{1}{q'} < \dfrac{b}{a}$, which means the sum of $n$ fractions $\leq \dfrac{1}{q'}$ can't reach $\dfrac{b}{a}$, hence it does not make sense to further decompose $x$. We stop and poll a pair from the \emph{head} of $C$; the pair contains $\dfrac{b'}{a'}$ and $Q$, from where we continue searching the best decomposition. Otherwise if $n\dfrac{1}{q'} \geq \dfrac{b}{a}$, we return to step 2), and continue try to decompose $x$ into no more than $n-1$ Egyptian fractions.
