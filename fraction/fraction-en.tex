...
It is the approximation of $\sqrt{2}$, with precision up to 6 places! While these numbers are ambiguous for lack of the point symbol, for example, the sequence 12, 15 may represent $12 \times 60 + 15 = 720$ or $12 + \dfrac{15}{60} = 12.25$. Moreover, the Babylonians didn't use zero in a unified way, people have to guess the exact value from the context.
...
After giving the notation of fractions, the author of \emph{Nine chapters} next dealt with the problem whether the value of a fraction was definitive, in other word, will multiple notations correspond to the same value? This led to the reduction operation and the concept of lowest terms (irreducible fraction). The author explained with examples:
...
The author explained, \enquote{let the sum of the products of the each denominator and the other nominator be the nominator of the result; let the product of the two denominators be the denominator of the result.} that is,

\be
\frac{b}{a} + \frac{d}{c} = \frac{bc + ad}{ac}
\ee

As the result may not be in its lowest terms, the author continued with a special case, \enquote{the result is 1 if the nominator equals the denominator,} and followed with the mixed number for the general case. At last the author addressed the special simple case of equal denominators, \enquote{one adds the nominators directly.}
