...
\vspace{3mm}
For Q2, we may reuse the method in \emph{Nine chapters}, comparing through the difference of $\dfrac{b}{a} - \dfrac{d}{c}$ with 0. It amounts to comparing the nominators after reducing them to the same denominators. As any whole number $n$ equals to $\dfrac{n}{1}$, we unify the comparison across whole numbers and fractions simply to comparison of fractions. The ordering across whole numbers, fractions, and decimals sorts out their positions on the number line.
...
Return to the legend of Pythagoras. After the discovery of the theory of music, that fractions ruled the harmonic sound, how to tune the seven strings of a lyre? If add the eighth string, then its length should be half of the first string, such that to get an octave. Pythagoras found overlapping a perfect fifth (ratio $3:2$) to a perfect four (ratio $4:3$) is an octave because $\dfrac{3}{2} \times \dfrac{4}{3} = 2$. He used the ratio $3:2$ of a perfect fifth as a basis scale, multiplied a geometric series:
